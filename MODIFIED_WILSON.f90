...
!------------------------------------------------------------------------
 ...
 integer::nels,neq,nband,nn,nr,nip,nodof=2,nod=4,nst=3,ndof,loaded_nodes,  &
          i,k,iel,ndim=2
 real:: e,v,det,mdet;  character(len=15) :: element='quadrilateral'  
!----------------------------- dynamic arrays---------------------------------
...
!---------------------------input and initialisation---------------------------
   open (10,file='Data\T15M.dat',status=    'old',action='read')
   open (11,file='Results\T15M.res',status='replace',action='write')                   
...
!---------------- element stiffness integration and assembly-------------------
 ...
 elements_2: do iel = 1 , nels
          ...
          gauss_pts_1: do i = 1 , nip
               ...
               call wilson_der (wder,points,i)
               jac = matmul(der,coord) 
               det = determinant(jac)
               call invert(jac)
               deriv = matmul(jac,der)
               wderiv = matmul(jac,wder)
               ! 计算 mdet 其中(xi,eta)取(0,0)
               point=0.0
               call shape_der (mder,point,i)
               mjac = matmul(mder,coord) 
               mdet = determinant(mjac)
               ! 计算 kuu
               call beemat (bee,deriv) 
               kuu = kuu + matmul(matmul(transpose(bee),dee),bee) *det* weights(i)
               ! 计算 kua 其中(xi,eta)取(0,0)
               call wilson_beemat (wbee,wderiv)
               kua = kua + matmul(matmul(transpose(bee),dee),wbee) *mdet* weights(i)
               ! 记录小片检验矩阵
               if(iel==1) ptc=ptc+wbee*mdet
               ! 计算 kaa
